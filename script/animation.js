...
function getCoord(e) {
    var carPos = titulo.selectionEnd,
        div = document.createElement('div'),
        span = document.createElement('span'),
        copyStyle = getComputedStyle(titulo),
        tituloCoords = {}, caretCoords = {}, centerCoords = {}
        ;
    ...
    div.textContent = titulo.value.substr(0, carPos);
    span.textContent = titulo.value.substr(carPos) || '.';
    div.appendChild(span);

    tituloCoords = getPosition(titulo);							//console.log("tituloCoords.x: " + tituloCoords.x + ", tituloCoords.y: " + tituloCoords.y);
    ...
    caretPos = caretCoords.x + tituloCoords.x;					//console.log("caretPos: " + caretPos);
    ...
    var eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, tituloCoords.x + caretCoords.x, tituloCoords.y + 25);
    var eyeLX = Math.cos(eyeLAngle) * eyeMaxHorizD;
    var eyeLY = Math.sin(eyeLAngle) * eyeMaxVertD;
    var eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, tituloCoords.x + caretCoords.x, tituloCoords.y + 25);
    var eyeRX = Math.cos(eyeRAngle) * eyeMaxHorizD;
    var eyeRY = Math.sin(eyeRAngle) * eyeMaxVertD;
    var noseAngle = getAngle(noseCoords.x, noseCoords.y, tituloCoords.x + caretCoords.x, tituloCoords.y + 25);
    var noseX = Math.cos(noseAngle) * noseMaxHorizD;
    var noseY = Math.sin(noseAngle) * noseMaxVertD;
    var mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, tituloCoords.x + caretCoords.x, tituloCoords.y + 25);
...
titulo.addEventListener('focus', onEmailFocus);
titulo.addEventListener('blur', onEmailBlur);
titulo.addEventListener('input', onEmailInput);
